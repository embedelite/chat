...
import { AIServiceStrategy, AnthropicStrategy, OpenAIStrategy } from "./ai-providers";
...
export type Model = "gpt-3.5-turbo" | "gpt-4" | "gpt-4-turbo-preview" | "gpt-4o" | "claude-3-5-sonnet-20240620";
...
export class ChatService {
    ...
    private getAIStrategy(model: Model): AIServiceStrategy {
        if (model.startsWith('gpt-')) {
            const apiKey = this.storageService.getItem<string>("oai_api_key");
            if (!apiKey) {
                throw new Error('OpenAI API Key not found');
            }
            return new OpenAIStrategy(apiKey, model);
        } else if (model.startsWith('claude-')) {
            const apiKey = this.storageService.getItem<string>("anthropic_api_key");
            if (!apiKey) {
    ...
    async sendMessage(
        ...
        if (chat.messages[0].from === "user") {
            const firstMessage = chat.messages[0].text;
            const title = firstMessage.length > 16 ? `${firstMessage.slice(0, 16)}...` : firstMessage;
        ...
        if (mode === "oai") {
            try {
                const aiStrategy = this.getAIStrategy(model);
                const stream = aiStrategy.sendMessage(message, history);
                ...
                for await (const chunk of stream) {
                    if ('text' in chunk) {
                        ...
                        // Update the chat in the BehaviorSubject
                        this.currentChatSubject.next({ ...chat });
                        // Optionally save to storage (consider performance impact)
                        // this.storageService.setItem("chats", this.chats);
