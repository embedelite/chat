...
@Component({
    ...
    <div class="w-full h-full flex items-start justify-center md:p-6 xs:p-2">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 w-4/5 max-h-full">
        <button
          type="button"
        ...
        <div class="px-6 py-6 lg:px-8">
          ...
          <form class="space-y-6" action="#" [formGroup]="configForm">
            ...
            <div>
              <label for="ee_api_key" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                EmbedElite API KEY
              </label>
              <input
            ...
            <div>
              <label for="oai_api_key" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                OpenAI API KEY
              </label>
              <input
            ...
            <!-- Anthropic API KEY Control -->
            <div>
              <label for="anthropic_api_key" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Anthropic API KEY
              </label>
              <input
                formControlName="anthropic_api_key"
            ...
            <div class="flex p-0 m-2">
              <label for="default_model" class="pt-3 pr-3 shrink-0 block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Default model:
              </label>
              <select
...
export class ConfigComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        const anthropic_api_key = this.storageService.getItem<string>("anthropic_api_key") || "";
        const defaultModel = this.storageService.getItem<string>("default_model") || "gpt-4";
        ...
        this.configForm = this.fb.group({
            ...
            anthropic_api_key: [anthropic_api_key, Validators.required],
    ...
    saveConfig() {
        this.storageService.setItem("oai_api_key", this.configForm.get("oai_api_key")?.value);
        this.storageService.setItem("ee_api_key", this.configForm.get("ee_api_key")?.value);
        this.storageService.setItem("anthropic_api_key", this.configForm.get("anthropic_api_key")?.value);
        this.storageService.setItem("default_model", this.configForm.get("defaultModel")?.value);
        this.toggleDarkMode();
